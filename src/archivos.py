...
def guardar_estado_sistema():
    ...
    estado = {
        ...
        # Asegurate de que en datos.py se llame 'escuderias_' o modificalo a 'escuderias'
        "escuderias": datos.escuderias,
        "carreras": datos.carreras,
        # Corregido a plural para que coincida
        "matriz_resultados": datos.matriz_resultados,
        # Asegurate que esto exista en datos.py
...
def exportar_reporte_txt():
    ...
    try:
        pilotos_ordenados = sorted(datos.pilotos.items(
        ), key=lambda item: item[1]["puntos"], reverse=True)
        escuderias_ordenadas = sorted(datos.escuderias.items(
        ), key=lambda item: item[1].get("puntos", 0), reverse=True)
        ...
        with open(ARCHIVO_REPORTE, "w", encoding="utf-8") as file:
            ...
            for pos, (sigla, info) in enumerate(escuderias_ordenadas, 1):
                ...
                file.write(
                    f"{pos:<4} | {sigla:<5} | {nombre:<30} | {puntos:<6}\n")
        ...
        console.print(
            f"[bold red]Error: No se pudo escribir el reporte de texto. {e}[/bold red]")


def restaurar_sistema_json():
    if not os.path.exists(ARCHIVO_JSON):
        console.print(
            f"[bold red]Error: No se encontro el archivo de respaldo '{ARCHIVO_JSON}'.[/bold red]")
        return
    console.print(
...
def menu_guardar():
    opciones_menu = ["1. Guardar estado del sistema",
                     "2. Exportar reporte", "0. Volver al menú principal"]
    while True:
        console.clear()
